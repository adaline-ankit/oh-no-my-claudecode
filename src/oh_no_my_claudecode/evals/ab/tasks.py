"""Built-in A/B eval task suite.

All tasks are self-contained: ``setup_script`` creates a minimal Python file
...
Why these tasks show ONMC value
--------------------------------
Each task has a plausible "wrong fix" that an agent without memory might
attempt.  The ONMC hint (injected only in cc_onmc) steers away from that
dead-end.  When the cc_alone condition stumbles into the wrong fix and
cc_onmc avoids it, that is the measurable delta.
