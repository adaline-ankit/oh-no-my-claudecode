...
- `onmc loop` invokes one installed Claude Code or Codex CLI through a headless adapter
...
ONMC owns memory, control, verification, and evidence. Claude Code or Codex owns code generation and
tool use. This keeps agent-specific execution replaceable and makes completion independently testable.
