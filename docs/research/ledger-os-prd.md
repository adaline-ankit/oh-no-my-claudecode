...
| M6 | **Retirement→rollback wiring** | 🔨 now | HARMFUL verdict ⇒ sink removal + gate rollback, one call |
| M7 | Hosted store (Supabase, RLS, pgvector) | ✅ shipped | retire = timestamp, never delete |
| M8 | Temporal validity windows (Graphiti-style) | next | stale memory queryable by time, auto-expires from routing |
...
| E4 | **Compaction-policy scoring** (lift per token freed) | 🔨 now | policies ranked with CIs on repo-bench fixtures |
...
| H9 | **Provable self-improvement demo** | 🔨 now | one script: trajectory→skill→lift→promote/retire→attested record |
| H10 | Sigstore keyless signer impl | next | rekor-logged signature verifies |
| H11 | Product-gate bytecode hardening | next | stale .pyc cannot false-red a live gate |
...
- **M-now (this wave):** M6 retirement wiring · E4 compaction scoring · H9 demo
