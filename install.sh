...
printf "%b===========================================================%b\n" "${BOLD}${GREEN}" "${RESET}"
printf "\n"
printf "Day-1 commands:\n"
printf "  %bonmc setup%b            -- re-run setup / hooks / MCP integration\n" "${BOLD}" "${RESET}"
printf "  %bonmc brief%b            -- what the repo knows before you touch code\n" "${BOLD}" "${RESET}"
printf "  %bonmc autopilot%b %b\"goal\"%b -- full KNOW->ACT->PROVE loop\n" "${BOLD}" "${RESET}" "${CYAN}" "${RESET}"
printf "  %bonmc ui%b               -- local dashboard\n" "${BOLD}" "${RESET}"
printf "\n"
